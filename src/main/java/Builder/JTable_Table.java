 ...
 *
 * Builder Pattern - Before - May be preferred since each table encapulates a different layout
 */
public class JTable_Table {
    private JTable_Table m_table;

    public JTable_Table(String[][] matrix) {
        m_table = new JTable_Table(matrix[0].length, matrix.length);
    ...
    public static String[][] read_data_file(String file_name) {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                int i = 0;
                if (first_line) {
                    matrix = new String[Integer.parseInt(tokens[0])][Integer.parseInt(tokens[1])];
                    i = 2;
                    first_line = false;
                }
